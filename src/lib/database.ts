...
export async function getDb(): Promise<Client> {
    ...
    while (retries > 0) {
        try {
            ...
            console.error(`Falha na conexão (tentativa ${3 - retries}/2):`, error.message);
            retries--;
...
async function ensureTablesExist(client: Client): Promise<void> {
    ...
    await client.query(`CREATE TABLE IF NOT EXISTS pixels (id UUID PRIMARY KEY, name VARCHAR(255) NOT NULL, provider VARCHAR(50) NOT NULL, code TEXT NOT NULL, content_id VARCHAR(255), active BOOLEAN DEFAULT TRUE, created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP);`);
...
async function insertDefaultData(client: Client): Promise<void> {
    ...
    const defaultRoutes = [
        ...
        { path: '/advinsu', name: 'Blog: Segredo da Insulina (Feminino)', content_id: 'advinsu' },
    ];
